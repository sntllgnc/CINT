...
R1 expresses these controls in strict TypeScript, but the emitted runtime is
JavaScript and the types are erased. The compiler is a development control,
not a trust boundary: every external record still requires the unchanged
runtime schema and cryptographic checks before use.

### Components and source

| Component | Security role | Source evidence |
|---|---|---|
| Canonical records | Execute all 13 public schemas; reject unknown fields, wrong protocols, and tampered digests | `src/cint/schema.ts`, `src/cint/canonical.ts` |
| Intent reconstruction | Bind request, action, target, context, effects, and uncertainty | `src/cint/intent.ts` |
| Authority and policy | Enforce exact grants, epochs, validity, allowed adapters, and rollback policy | `src/cint/authority.ts`, `src/cint/policy.ts` |
| Counter-intent and decision | Deny silent or mismatched intent; route uncertainty; keep decisions non-executable | `src/cint/challenge.ts`, `src/cint/decision.ts` |
| Receipt authority | Authenticate eligible decision bindings and lifetime | `src/cint/receipt.ts` |
| Receipt store | Register, lock, revalidate, consume once, and terminally reject | `src/cint/store.ts` |
| Execution gate | Preflight verifiers, revalidate inside the lock, before preparation, and immediately after preparation before execution | `src/cint/execution.ts` |
| Outcome, ledger, and seal | Verify or restore state; hash-chain and authenticate terminal evidence | `src/cint/outcome.ts`, `src/cint/evidence.ts`, `src/cint/seal.ts` |
| Synthetic adapter | Confine a disposable patch and restore exact original bytes | `src/cint/adapters/synthetic-file-patch.ts` |
| Codex Adapter 01 | Bind and execute the preserved read-only Agent Floor packet through a typed boundary | `src/cint/adapters/codex/index.ts`, `src/cint/adapters/codex/legacy-adapter-boundary.ts` |
...
|---|---|---|---|---|---|---|
| Embedded CINT runtime | Receipt key | Constructor input owned by embedding process | Private in-memory field; never serialized | Receipt authority only | Minimum 32 bytes and HMAC-SHA256 | `src/cint/receipt.ts`; external key storage and rotation are caller-owned |
| Embedded CINT runtime | Seal key | Constructor input owned by embedding process | Private in-memory field; never serialized | Seal authority only | Minimum 32 bytes and HMAC-SHA256 | `src/cint/seal.ts`; external key storage and rotation are caller-owned |
| One-shot execution | Receipt state root | Caller supplies store root | Hashed filenames under pending, locks, consumed, rejected | CINT store process | Exclusive creation and terminal records | `src/cint/store.ts` |
| Evidence workflow | Ledger file | Caller supplies file path | Canonical local JSONL with adjacent exclusive lock | CINT execution gate | Digest chain, exclusive append, fsync | `src/cint/evidence.ts`; retention is caller-owned |
| Synthetic proof | Writable target | Adapter root plus receipt-bound relative path | Existing regular file resolved inside disposable root | Synthetic adapter only | Containment, expected pre-hash, atomic write, verify, rollback | `src/cint/adapters/synthetic-file-patch.ts` |
...
| Codex Adapter 01 | Run evidence directory | Adapter constructor input | Caller-selected local output directory | Legacy runner and local operator | Adapter task/packet digest plus CINT receipt | `src/cint/adapters/codex/index.ts`; output retention and disclosure are caller-owned |
...
- CINT trusts the selected in-process adapter implementation to honor its signed
...
- Strict TypeScript compilation is trusted to detect declared type violations
  during development; it is not trusted to establish runtime input validity.
...
| P0 | Adapter reports success despite divergent state | Buggy or hostile adapter result | False terminal success | Independent adapter verification; synthetic divergence invokes rollback; core seal requires verified/restored outcome | Add independent verifiers for future consequential adapters | `src/cint/execution.ts`, `src/cint/outcome.ts` |
| P1 | Crash leaves receipt lock behind | Process interruption while holding lock | Local denial of service, not unauthorized execution | Ambiguous lock remains fail closed | Define a separately authorized recovery receipt and operator procedure before production use | `src/cint/store.ts`; crash-lock test |
| P1 | Tamper with ledger or terminal evidence | Local filesystem write access | Audit confusion | Canonical entries, sequence, previous digest, evidence seal over ledger head | Store sealed bundles on append-only or separately protected storage | `src/cint/evidence.ts`, `src/cint/seal.ts` |
