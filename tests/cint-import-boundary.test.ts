...
import { errorTextIncludes, parseJsonRecord } from "./cint-test-support.js";
...
async function runDeniedImportProbe(arguments_: readonly string[]) {
...
test("the identity CLI loads without Adapter 01 or the legacy CLI", async () => {
  ...
  assert.equal(parseJsonRecord(stdout)["product_code"], "CINT");
});
...
test("the legacy CLI crosses the adapter boundary only when explicitly invoked", async () => {
  await assert.rejects(
    ...
    (error: unknown) => errorTextIncludes(error, "stderr", "CINT_IMPORT_BOUNDARY")
  );
});
